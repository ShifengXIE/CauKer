...
def main(
    ...
    # # 1. 构造阶段相关参数
    # num_stages = len(training_data_paths)
    # steps_per_stage = max_steps // num_stages  # 每阶段步数，200000/5=40000
    # cumulative_steps = 0

    # # 2. 使用同一个 Trainer 实例，依次替换 train_dataset 并累积训练步数
    # trainer = Trainer(
    #     model=model,
    #     args=training_args,
    #     # 初始填入占位数据集，稍后在循环中替换
    ...
    # for stage_idx, data_path in enumerate(training_data_paths, start=1):
    #     # 2.1 构造当前阶段的 ChronosDataset
    #     stage_dataset = ChronosDataset(
    #         datasets=[FileDataset(path=Path(data_path), freq="h")],
    #         probabilities=[1.0],
    #         tokenizer=chronos_config.create_tokenizer(),
    #         context_length=context_length,
    #         prediction_length=prediction_length,
    ...
    #         model_type=model_type,
    #         imputation_method=(
    #             LastValueImputation() if model_type == "causal" else None
    #         ),
    #         mode="training",
    #     ).shuffle(shuffle_buffer_length=shuffle_buffer_length)

    #     # 2.2 替换 Trainer 的 train_dataset
    #     trainer.train_dataset = stage_dataset

    #     # 2.3 更新目标步数
    #     # target_steps = cumulative_steps + steps_per_stage
    #     target_steps = steps_per_stage
    #     trainer.args.max_steps = target_steps

    #     log_on_main(
    #         f"[Stage {stage_idx}/{num_stages}] Training on {data_path} for {steps_per_stage} steps (up to global step {target_steps})",
    #         logger,
    #     )

    #     # 2.4 继续训练（恢复自上一次 checkpoint，无重置 optimizer 或 scheduler）
    #     trainer.train()
